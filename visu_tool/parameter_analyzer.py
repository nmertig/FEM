...
class ParameterAnalyzer(object):
    """
    """
    def __init__(self, kappa, hinv, parameter_array, parameter_name,
                 data_path, N_evals, x_plot_fct):
        """
        """
        self.kappa = kappa
        self.hinv = hinv
        ...
        self.N_params = len(parameter_array)

        self.data_path = data_path

        self.N_evals = N_evals
        self.x_plot_fct = x_plot_fct
    ...
    # ######################################################################
    def create_fname_array(self):
        """
        """
        self.fname_array = [] # strictly speaking it is a list

        for i in range(self.N_params):
            file_id = "kappa_" + "{:.5f}".format(self.kappa)\
                    + "_hinv_" + "{:.5f}".format(self.hinv)\
                    + "_" + self.parameter_name + "_" \
                    + "{:.5f}".format(self.parameter_array[i]) + ".npy"
            self.fname_array += [file_id]

    def create_x_plot(self):
        """params = [kappa, hbar, max_order, N, x_max, degree, degree2D]
        """
        comp_params = np.load(self.data_path + "params/" +
                              self.fname_array[self.i])
        self.x_plot = self.x_plot_fct(comp_params)
    ...
    def load_eval_array(self):
        ...
        """
        self.eval_array = np.ones((self.N_params, self.N_evals), dtype=complex)
        for i in range(self.N_params):
    ...
    def _plot_gammas(self):
        ...
        if self.k < self.N_max:
            self.ax3.plot([self.parameter_array[self.i]],
        ...
        for m in np.arange(self.N_max)[::-1]:
            ...
            self.ax3.plot(self.parameter_array[i_],
    ...
    def _plot_phases(self):
        ...
        # current data point ------------------------
        if self.k < self.N_max:
            self.ax4.plot([self.parameter_array[self.i]],
        ...
        for m in np.arange(self.N_max)[::-1]:
            i_ = i_array[self.i_min_m[m]:]
            self.ax4.plot(self.parameter_array[i_],
    ...
    def _save_diabatic_line(self):
        ...
        np.save(self.data_path + "/pp/" + fname + "_evals_m_" + str(self.m),
                [params, evals])